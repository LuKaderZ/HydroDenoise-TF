...
from model import DCAMFNet, bias
...
def run_evaluation(model, test_dir, sample_rate, device):
    ...
    fnames = sorted([f for f in os.listdir(noisy_dir) if os.path.splitext(f)[1].lower() in exts])
    ...
    for fname in tqdm(fnames, desc=f"Testing {os.path.basename(test_dir)}", unit="file"):
        ...
        torchaudio.save(os.path.join(denoised_path,fname), torch.from_numpy(e_np), sample_rate)
    ...
    final_metrics = {
        ...
        'SISNR': np.mean(results['sisnr_a']) + bias,
        'SISNRi': np.mean(np.array(results['sisnr_a']) - np.array(results['sisnr_b'])) + bias,
        'SDR': np.mean(results['sdr_a']) + bias,
        'SDRi': np.mean(np.array(results['sdr_a']) - np.array(results['sdr_b'])) + bias
    }
