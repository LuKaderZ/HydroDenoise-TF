...
class AudioDenoisingDataset(Dataset):
    ...
    def __init__(self, root_dir, sample_rate=16000, 
                 segment_seconds=3.0, overlap_seconds=1.0):
        super().__init__()
        self.sample_rate = sample_rate
        self.segment_len = int(sample_rate * segment_seconds)      # 48000 (3s)
        self.hop_len = int(sample_rate * (segment_seconds - overlap_seconds))  # 32000 (2s)
        ...
        for pair_idx, (n_path, _) in enumerate(tqdm(self.pairs, desc="预查音频长度")):
            ...
            if n_samples_at_target <= self.segment_len:
                ...
                n_segs = math.ceil((n_samples_at_target - self.segment_len) / self.hop_len) + 1
        ...
        print(f"[Dataset] 加载完成: {len(self.pairs)} 条原始音频 -> {len(self.segments)} 个 3s 片段")
    ...
    def _load_segment(self, path, start_frame):
        ...
        try:
            audio, _ = sf.read(path, start=start_frame, frames=num_frames_to_read, dtype='float32')
        ...
        if waveform.shape[-1] < self.segment_len:
            waveform = torch.nn.functional.pad(waveform, (0, self.segment_len - waveform.shape[-1]))
        else:
