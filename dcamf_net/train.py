...
def main(args):
    ...
    full_dataset = AudioDenoisingDataset(root_dir=args.train_dir, sample_rate=args.sample_rate)
    ...
    if args.use_tensorboard:
        log_dir = args.log_dir if args.log_dir else os.path.join(args.save_dir, 'tensorboard')
    ...
    for epoch in range(args.epochs):
        train_res = train_one_epoch(model, train_loader, criterion, optimizer,
        ...
        log_msg = (f"Epoch [{epoch + 1:03d}] | LR: {curr_lr:.2e} | "
                   f"Train [Loss: {train_res['loss']:.4f}, SISNR: {train_res['sisnr']:.2f}dB] | "
                   f"Val [Loss: {val_res['loss']:.4f}, SISNR: {val_res['sisnr']:.2f}dB]")
        logger.info(log_msg)

        if writer is not None:
            writer.add_scalars('Metrics/Loss', {'train': train_res['loss'], 'val': val_res['loss']}, epoch)
            writer.add_scalars('Metrics/SISNR', {'train': train_res['sisnr'], 'val': val_res['sisnr']}, epoch)
            writer.add_scalars('Metrics/SDR', {'train': train_res['sdr'], 'val': val_res['sdr']}, epoch)
        ...
        if val_res['loss'] < best_val_loss:
            ...
            logger.info(f"  -> Best model (loss) saved with val_loss={best_val_loss:.4f}")

        if val_res['sisnr'] > best_val_sisnr:
            best_val_sisnr = val_res['sisnr']
            torch.save(model.state_dict(), os.path.join(args.save_dir, 'best_SISNR.pth'))
            logger.info(f"  -> Best model (SISNR) saved with val_sisnr={best_val_sisnr:.2f}dB")

    logger.info(f"Training complete. Best val loss: {best_val_loss:.4f}, Best val SISNR: {best_val_sisnr:.2f}dB")
...
def parse_args():
    ...
    parser.add_argument('--T_0', type=int, default=200)      # 第一个周期长度，大于epochs则无重启
