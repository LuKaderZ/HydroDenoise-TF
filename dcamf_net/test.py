...
def parse_args():
    ...
    parser.add_argument('--test_dir', type=str, nargs='+', required=True, help='一个或多个测试集路径')
...
def main():
    ...
    for t_dir in args.test_dir:
        ...
        if metrics:
            ...
            print(f"   SISNR:  {metrics['SISNR']:.4f} dB (SISNRi: {metrics['SISNRi']:.4f})")
...
if __name__ == '__main__':
    import sys
    sys.argv = ['test.py','--checkpoint','checkpoints/best_SISNR.pth' ,'--test_dir','data/ShipsEar/train']
    main()
    '''
