%% 图4-2：各模型降噪前后平均功率谱密度对比 (三条线，降噪信号为视觉焦点，0-4 kHz)
clear; clc; close all;
...
for k = 1:nSamples
    ...
    % 干净信号
    [clean, ~] = audioread(fullfile(cleanDir, fname));
    clean = mean(clean, 2);

    % 原始带噪信号
    noisyFile = fullfile(noisyDir, fname);
    if ~exist(noisyFile, 'file'), continue; end
    [noisy, ~] = audioread(noisyFile);
    noisy = mean(noisy, 2);
    ...
    minLen = min([length(clean), length(noisy), length(crn_est), ...
                  length(convtasnet_est), length(dprnn_est), length(dcamf_est)]);
    ...
    [pxx_noisy, ~] = pwelch(noisy, window, noverlap, nfft, fs);
    ...
    if isempty(avg_psd_clean)
        ...
        avg_psd_noisy = avg_psd_noisy + pxx_noisy;
...
avg_psd_noisy       = 10*log10(avg_psd_noisy / validCount);
...
% ==================== 统一坐标轴范围 ====================
allPSD = [avg_psd_clean; avg_psd_noisy; avg_psd_crn; avg_psd_convtasnet; avg_psd_dprnn; avg_psd_dcamf];
yMin = floor(min(allPSD) / 10) * 10;
yMax = ceil(max(allPSD) / 10) * 10;
xLimits = [0 4];  % kHz，聚焦能量集中频段
...
models = {
    'CRN',              avg_psd_crn;
    'Conv-TasNet',      avg_psd_convtasnet;
    'DPRNN',            avg_psd_dprnn;
    'DCAMF-Net',        avg_psd_dcamf;
};
...
for i = 1:4
    ...
    % 1. 干净信号 —— 浅灰色细实线，作为参考背景
    plot(f/1000, avg_psd_clean, '-', 'Color', [0.5 0.5 0.5], 'LineWidth', 0.6);

    % 2. 原始带噪信号 —— 浅灰色点线，退居背景
    plot(f/1000, avg_psd_noisy, ':', 'Color', [0.5 0.5 0.5], 'LineWidth', 0.6);

    % 3. 降噪后信号 —— 黑色粗虚线加圆圈标记，视觉焦点
    plot(f/1000, models{i,2}, 'k-', 'LineWidth', 1.2, 'MarkerSize', 3, ...
         'MarkerFaceColor', 'k', 'MarkerIndices', 1:25:length(f));
