%% 图4-5：各模型线谱功率恢复条形图 (基于局部突出度自动选峰，最佳样本，输出数据)
clear; clc; close all;
...
pdfPath = fullfile(saveDir, 'fig4-5_Line_Spectra_Bar.pdf');
pngPath = fullfile(saveDir, 'fig4-5_Line_Spectra_Bar.png');
exportgraphics(gcf, pdfPath, 'ContentType', 'vector');
saveas(gcf, pngPath);
fprintf('\n图4-5 线谱功率恢复条形图已保存至 %s\n', saveDir);
