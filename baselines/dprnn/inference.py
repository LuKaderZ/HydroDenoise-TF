...
MODEL_PATH = os.path.join(PROJECT_ROOT, "experiments", "dprnn", "checkpoints", "best_model.pth")
...
def run_inference(data_dir, est_dir, desc="推理"):
    ...
    model = DPRNNTasNet(**MODEL_KWARGS).to(DEVICE)
...
def main():
    ...
    EST_DIR  = os.path.join(PROJECT_ROOT, "experiments", "dprnn", "estimates", "tt_test1")
    run_inference(DATA_DIR, EST_DIR, desc="推理 test1")

    # ---- DeepShip ----
    DATA_DIR = os.path.join(PROJECT_ROOT, "data", "DeepShip", "test")
    EST_DIR  = os.path.join(PROJECT_ROOT, "experiments", "dprnn", "estimates", "tt_DeepShip")
